...
> **Windows の場合：** `python3` コマンドが使えない場合は  
> 付属の `python3.bat` ラッパー経由で実行できます。

### ⚠️ 本番実行前に必ずドライランで件数確認

**まず --dry-run で確認してください（APIコスト一切なし）：**
...
> **ドライランをスキップして直接本番実行するとAPI費用が予期せず発生する恐れがあります。**  
> 特に Gmail のメルマガは1通あたり20〜30件の記事候補があるため、必ず事前確認してください。
